...
def run(settings):
    ...
    davis_train = Davis(version='2016', multiobj=False, split='train')
    davis_val = Davis(version='2016', multiobj=False, split='val')
    ...
    dataset_train = sampler.LWLSampler([davis_train], [1],
                                       samples_per_epoch=settings.batch_size * 1000, max_gap=100, num_test_frames=3,
                                       num_train_frames=1,
                                       processing=data_processing_train)
    dataset_val = sampler.LWLSampler([davis_val], [1],
                                     samples_per_epoch=settings.batch_size * 100, max_gap=100,
                                     num_test_frames=3,
                                     num_train_frames=1,
                                     processing=data_processing_val)
    ...
    #==change
    weights_path = '/home/gaoshang/pytracking/pretrained_networks/e2e_mask_rcnn_R_50_FPN_1x_converted.pkl'
    #==change
