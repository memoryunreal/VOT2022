...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, run_id: int = None, display_name: str = None, ax_tracker='dimp_ax'):
        ...
        self.ax_parameter_name = ax_tracker
    ...
    def run_vot2022d(self, debug=None, visdom_info=None):
        params = self.get_parameters()
        ax_params = self.get_parameters(use_ax = True)

        params.tracker_name = self.name
        params.param_name = self.parameter_name
        ax_params.param_name = self.ax_parameter_name
        params.run_id = self.run_id
        ...
        tracker = self.create_tracker(params)
        ax_tracker = self.create_tracker(ax_params)
        tracker.initialize_features()
        ax_tracker.initialize_features()
        ...
        image_path = _convert_image_path(image_path)
        # print(image_path)
        colormap = self._read_depth_image(image_path)
        depth = self._read_depth_image(image_path, colormap=False)
        output_segmentation = False
        if output_segmentation:
        ...
        out = tracker.initialize(colormap, info={'init_mask': vot_anno_mask, 'init_bbox': bbox})
        ax_out = ax_tracker.initialize(depth, info={'init_mask': vot_anno_mask, 'init_bbox': bbox})

        if out is None:
            out = {}
        prev_output = OrderedDict(out)
        ax_prev_output = OrderedDict(ax_out)
        ...
        while True:
            ...
            ax_info = OrderedDict()
            ax_info['previous_output'] = ax_prev_output

            out = tracker.track(colormap, info=info)
            ax_out = ax_tracker.track(depth, info=ax_info)

            

            

            if out['max_score']<0.3 and ax_out['max_score']>0.8:
                out = ax_out
            prev_output = OrderedDict(out)
            ax_prev_output = OrderedDict(ax_out)
    ...
    def get_parameters(self, use_ax=False):
        """Get parameters."""
        if use_ax == False:
            param_module = importlib.import_module('pytracking.parameter.{}.{}'.format(self.name, self.parameter_name))
            params = param_module.parameters()
        else:
            param_module = importlib.import_module('pytracking.parameter.{}.{}'.format(self.name, self.ax_parameter_name))
            params = param_module.parameters()
    ...
    def _read_depth_image(self, depth_file: str,colormap = True):
        if colormap:
