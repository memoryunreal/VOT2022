...
def torch_to_numpy(im, normalize=False):
    mean = np.array([0.485, 0.456, 0.406]).reshape(3, 1, 1)
    std = np.array([0.229, 0.224, 0.225]).reshape(3, 1, 1)

    im = im.clone().cpu().squeeze().numpy()
    ...
    im = np.ascontiguousarray(im.transpose(1, 2, 0).astype(np.uint8))
...
# !OUTDATED! (it seems)
def show_image_state(im, bb_gt, bb_proposals, iou_pred=None, im_title=None, normalize=True):
    if im_title is None:
        im_title = 'ImageState'
    im_np = torch_to_numpy(im, normalize)

    disp_image_size = (640, 640)
    resize_factor = (disp_image_size[0] / im_np.shape[0], disp_image_size[1] / im_np.shape[1])

    bb_gt = bb_gt.clone().squeeze().cpu().numpy()
    bb_proposals = bb_proposals.clone().squeeze().cpu().numpy()

    bb_gt = (bb_gt * np.array(resize_factor + resize_factor)).round().astype(int)
    bb_proposals = (bb_proposals * np.array(resize_factor + resize_factor).reshape(1, 4)).round().astype(int)

    im_disp = cv2.resize(im_np, disp_image_size)

    # Draw proposals
    for i_ in range(bb_proposals.shape[0]):
        bb = bb_proposals[i_, :]
        disp_color = (i_*38 % 256, (255 - i_*97) % 256, (123 + i_*66) % 256)
        cv2.rectangle(im_disp, (bb[0], bb[1]), (bb[0] + bb[2], bb[1] + bb[3]),
                      disp_color, 1)

        if iou_pred is not None:
            text_pos = (bb[0], bb[1] - 5)
            cv2.putText(im_disp, 'ID={} IOU = {:3.2f}'.format(i_, iou_pred[i_]), text_pos,
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1, bottomLeftOrigin=False)

    # Add GT
    cv2.rectangle(im_disp, (bb_gt[0], bb_gt[1]), (bb_gt[0] + bb_gt[2], bb_gt[1] + bb_gt[3]), (0, 0, 255), 2)


    cv2.imshow(im_title, im_disp)
    cv2.waitKey(10)