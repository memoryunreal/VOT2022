...
class SoftmaxCEProblem:
    """Base class for representing an L2 optimization problem."""

    def __init__(self, sample_weights, softmax_reg=None, hes_reg=0.01):
        self.sample_weights = sample_weights
        self.softmax_reg = softmax_reg
        self.hes_reg = hes_reg
    ...
    def scores_softmax(self, s):
        return softmax_reg(s.reshape(*s.shape[:-2], -1), dim=-1, reg=self.softmax_reg).reshape(s.shape)

    def get_residuals(self, x: TensorList):
        raise NotImplementedError

    def data_grad(self, sm_scores):
        return self.sample_weights.view(-1,1,1,1) * (sm_scores - self.get_label_density())

    def apply_hessian(self, scores_grad, scores_softmax):
        sm_scores_grad = scores_softmax * scores_grad
        hes_scores_grad = sm_scores_grad - scores_softmax * torch.sum(sm_scores_grad, dim=(-2,-1), keepdim=True) + self.hes_reg * scores_grad
        hes_scores_grad = self.sample_weights.view(-1,1,1,1) * hes_scores_grad
        return hes_scores_grad

    def get_loss(self, s, res):
        exp_reg = 0 if self.softmax_reg is None else math.exp(self.softmax_reg)
        loss = torch.sum(self.sample_weights.view(-1, 1) *
                         (torch.log(s.exp().sum(dim=(-2, -1)) + exp_reg) - (self.get_label_density() * s).sum(dim=(-2, -1)))) + \
               0.5*self.ip_output(res, res)
        return loss
...
class SoftmaxCECG(ConjugateGradientBase):
    """Gauss-Newton with Conjugate Gradient optimizer."""

    def __init__(self, problem: SoftmaxCEProblem, variable: TensorList, cg_eps = 0.0, fletcher_reeves = True,
                 standard_alpha = True, direction_forget_factor = 0, debug = False, analyze = False, plotting = False,
                 visdom=None):
        super().__init__(fletcher_reeves, standard_alpha, direction_forget_factor, debug or analyze or plotting)
    ...
    def run(self, num_cg_iter, num_gn_iter=None):
        ...
        if isinstance(num_cg_iter, int):
            if num_gn_iter is None:
                num_gn_iter = 0 if num_cg_iter == 0 else 1
            num_cg_iter = [num_cg_iter]*num_gn_iter
        ...
        # Outer loop for running the GN iterations.
        for cg_iter in num_cg_iter:
            self.run_outer_iter(cg_iter)

        if self.debug:
            if not self.analyze_convergence:
                s = self.problem.get_scores(self.x)
                res = self.problem.get_residuals(self.x)
                loss = self.problem.get_loss(s, res)
                self.losses = torch.cat((self.losses, loss.detach().cpu().view(-1)))
    ...
    def run_outer_iter(self, num_cg_iter):
        """Runs a single GN iteration."""
        ...
        s = self.problem.get_scores(self.x)
        s_dt = s.detach()
        self.sm = self.problem.scores_softmax(s_dt)
        res = self.problem.get_residuals(self.x)
        self.f0 = TensorList([s]).concat(res)

        # Create copy with graph detached
        score_grad = self.problem.data_grad(self.sm)
        self.g = TensorList([score_grad]).concat(res.detach())

        if self.debug and not self.analyze_convergence:
            loss = self.problem.get_loss(s_dt, res.detach())
            self.losses = torch.cat((self.losses, loss.detach().cpu().view(-1)))
    ...
    def A(self, x):
        dfdx_x = TensorList(torch.autograd.grad(self.dfdxt_g, self.g, x, retain_graph=True))
        dfdx_x[0] = self.problem.apply_hessian(dfdx_x[0], self.sm)
        return TensorList(torch.autograd.grad(self.f0, self.x, dfdx_x, retain_graph=True))
